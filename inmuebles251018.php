	...
	<div class="barra_tipos">
		<?php
			...
			while($un_tipo_inm=$resultado_tipos_inm->fetch_row())
			{
				...
				echo '<div class="barra_un_tipo" id="'.$un_tipo_inm[0].'" draggable="true"><div class="div_inm" draggable="true"><div class="cabecerainm" style="background-color:'.$colores_estado_disponible[0].';color:'.$colores_estado_disponible[2].'"><input type="hidden" class="oculto" value="'.$proyecto.':'.$un_tipo_inm[0].':'.$un_reg_estado[0].'"><div id="divlogo"></div>'.substr($un_tipo_inm[1],0,5).'</div><div id="contenido_caja_inm" style="background-color:'.$colores_estado_disponible[1].';color:'.$colores_estado_disponible[3].'"><span class="clase_usr"></span><div class="acronimo_estado_inm">'.$un_reg_estado[2].'</div><span class="clase_cli"></span></div></div></div>';
			}
		?>
...
<div id="cabecera_pagina"><form name="form1" method="post" action="inmuebles.php" id="form1">
  ...
  ?>
      ...
      <td width="15%" align="right">Usuario : <?php echo $nombre;?> | <a href="javascript:salir()">Salir</a></td>
    </tr>
  </table>
</form>
<table id="tb_titulo">
...
<table width="100%" border="0" id="tb_padre">
  <tr>
    <td valign="top"><table width="100%" border="0">
        <tr>
          <td width="33%"><a href="javascript:masfila();"  class="masfila">+fila</a></td>
          <td width="33%">
          <?php
		  ...
		  while($uncolor=$resultadocolores->fetch_row())
		  {
			...
			echo "<td bgcolor='".$esquema_color[1]."'><div class='clase_estado_inm' style='color:".$esquema_color[3]."'>".$uncolor[1]."[".$cant."]</div></td>";
		  }
...
<?php
	  ...
	  else
	  {
		...
		while($un_registro=$inmuebles->fetch_row())
		{
			...
			$matriz[$ff][$cc]='<td><div class="div_inm" id="id_div_'.$ff.'_'.$cc.'" draggable="true" oncontextmenu="return muestra_menu_contextual(event,this)"><div class="cabecerainm" style="background-color:'.$color_celda[0].';color:'.$color_celda[2].'"><input type="hidden" name="id_inm_'.$ff.'_'.$cc.'" id="id_inm_'.$ff.'_'.$cc.'" value="'.$id_inm.'"><div id="divlogo"></div>'.$tipo_inmueble.'<a class="elim_inm">x</a></div><div id="contenido_caja_inm" style="background-color:'.$color_celda[1].';color:'.$color_celda[3].'"><div class="clase_usr">'.$mostrar_usr.'</div><div class="acronimo_estado_inm">'.$nombre_estado_actual.'</div><div class="clase_cli">'.$mostrar_cliente.'</div></div></div></td>';
		}
		//print_r($matriz);
		echo '<table id="tb1">';
